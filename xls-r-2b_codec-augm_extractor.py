...
def augment_audio(input_file):
        ...
        command = [
                'ffmpeg', '-y', '-i', input_file, '-acodec', codec,
                '-b:a', bitrate, '-strict', '-2','-loglevel', 'quiet', temp_output
        ]
...
def extract_features_codec(outdir,indir, metadata_file):
    ...
    np.save(os.path.join(outdir, f'{model_name}_feats_codec_augm.npy'), stacked_embeddings)
