...
class HuggingFaceFeatureExtractor:
    ...
    def __call__(self, audios, srs):
        # Feature extractor can process a list of audios at once
        inputs = self.feature_extractor(
            audios,
            sampling_rate=srs[0],  # assume all same sr
            return_tensors="pt",
            padding=True,
        )
...
def collate_fn(batch):
    """Batch list of (audio, sr, filename)."""
    audios, srs, filenames = zip(*batch)
    return list(audios), list(srs), list(filenames)


def main(outdir, indir, metadata_file, batch_size=8, num_workers=4):
    ...
    np.save(
        os.path.join(outdir, "wav2vec2-xls-r-2b_Layer9_asv19_eval.npy"),
        stacked_embeddings,
    )
...
if __name__ == "__main__":
    ...
    indir = "/ds-slt/audio/ASVSpoof_LA_19/ASVspoof2019_LA_eval/flac/"
    outdir = "./feats/wav2vec2-xls-r-2b/"
    metadata_file = "./processed_metadata/asv19_eval_systems.csv"
