...
def load_dataset(indices, meta_dir, metadata, feats_dir, feats):
    ...
    for index in indices:
        with open(os.path.join(meta_dir, metadata[index])) as fin:
            lines = fin.readlines()
            labels = [
                1 if line.strip().split("|")[1] == "bonafide" else 0 for line in lines
            ]
...
X_asv19_train, Y_asv19_train, _, _ = load_dataset(
    train_groups["asv19_train"], meta_dir, metadata, feats_dir, feats
)
X_asv19_dev, Y_asv19_dev, _, _ = load_dataset(
    train_groups["asv19_dev"], meta_dir, metadata, feats_dir, feats
)
...
for group_name, group_indices in all_eval_groups.items():
    X_eval, Y_eval, _, _ = load_dataset(
        group_indices, meta_dir, metadata, feats_dir, feats
    )
