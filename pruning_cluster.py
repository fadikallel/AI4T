    ...
    for label in np.unique(y):
        ...
        if order == "ascending":
            sorted_distances = np.sort(
                class_distances, order="distance"
            )  ## ascending order (close points)
        elif order == "descending":
            sorted_distances = np.sort(class_distances, order="distance")[
                ::-1
            ]  # descending sorting (further point)
        else:
            raise ValueError(
                f"{order} is not a valid order, please choose from 'ascending' or 'descending'"
            )
...
for pruning_fraction in pruning_fractions:
    ...
    with open("./xls_results_ALL_pruning_cluster.txt", "a") as f:
        f.write(
            f"\tpruning fraction: {pruning_fraction}, itw_eer:{eer_itw * 100:.2f}, ai4trust_eer:{eer_ai4trust * 100:.2f}"
            + "\n"
        )
        f.flush()
